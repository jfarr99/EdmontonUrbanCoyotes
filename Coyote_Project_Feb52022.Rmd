...
rm(hab.expected, hab.totals, hab.observed, hab.plot, table.hab, hab.chsq)
...
## Table 1. Contextual Patterns in Coyote Reports
...
act.reports = Coyotes %>%
  ...
  mutate(Categories = fct_relevel(Categories, "Unknown", "HomeYard", "Driving", "Cycling", "OutdoorAct", "Walking"), 
         Categories = recode(Categories, HomeYard = 'Home/Yard', OutdoorAct = 'Outdoor Activity')) %>%
...
vul.reports = Coyotes %>%
  ...
  mutate(VulnerableIndividual = fct_relevel(VulnerableIndividual,  c("Unknown", "Multiple", "Child", "Dog", "Cat")), 
         Categories = VulnerableIndividual) %>%
  dplyr::select(c(Variable, Categories, Count, perc))
...
leash.reports = Coyotes %>% filter(DogPresent !="HomeYard") %>% filter(VulnerableIndividual =="Dog"| VulnerableIndividual =="Multiple")%>%
  ...
  Categories = recode(Categories, OffLeash = "Off-Leash")) %>%
  dplyr::select(c(Variable, Categories, Count, perc))
...
num.reports = Coyotes %>%
  ...
  mutate(Categories = CoyoteNumber, 
         Categories = fct_relevel(Categories,  c("Unknown", "More", "Three", "Two", "One"))) %>%
    dplyr::select(c(Variable, Categories, Count, perc))
...
health.reports = Coyotes %>%
  ...
  mutate(Categories = fct_relevel(Categories, c("Unknown", "Unhealthy","Healthy"))) %>%
    dplyr::select(c(Variable, Categories, Count, perc))
...
save_plot("Figures/distribution_of_reports_across_contextual_vars.jpeg", context.plot.trends, base_width = 6, base_height= 6)

write.csv(context.reports,"Output_Tables/report_distrib_across_contexutal_vars.csv") # write that csv!


rm(act.reports, vul.reports, leash.reports, num.reports, health.reports)
...
## Figure 3. Changes in coyote boldness and human concern over years
...
save_plot("Figures/boldness.concern.over.years.plot.jpeg", years.plot, base_width = 10, base_height= 10)

rm(bold.time, perc.time, bold.time.plot, perc.time.plot)

```


## Figure 4. Contextual variables associated with coyote boldness and human concern
...
Now plot a bunch of those contextual variables together to make figure 4
```{r}
...
rm(bold.scale_models, bold.scale.model.outputs, null)

```

Fourth, we evaluate correlations between our variables using R-squared tests. Strong correlations between building density/road dist decay and Res100m. So, because res100m has the bigger (worst) AIC value, we remove it from analyses going forward. We also write a .csv file with corr output
```{r}
bold.correlations = rcorr(as.matrix(bold.data.trans[,c("RoadDistDecay", "Mod400m", "Nat100m", "Mow100m", "Build200m", "Com200m", "Res100m")]), type = c("spearman"))
...
bold.model.base = clm(Bold ~ Road_Distance_Decay + Natural + Modified_Open + Mowed + Commercial + Building_Density + Season + 
                        Road_Distance_Decay*Season + Natural*Season + Modified_Open*Season + Mowed*Season + 
                        Commercial*Season + Building_Density*Season, 
                      data = bold.data.modelling, na.action = "na.fail")
AIC(bold.model.base)- AIC(bold.model.null)

summary(bold.model.base)
bold.model.base.dredge = dredge(bold.model.base) # dredging base models to get top base models 

bold.model.base.dredge.coefs <- model.avg(bold.model.base.dredge, subset=delta < 2) # get the top model
summary(bold.model.base.dredge.coefs) # see all coefs 
bold.model.base.dredge.variables <- get.models(bold.model.base.dredge, subset=delta ==0) # get the top model
summary(bold.model.base.dredge.variables[["622"]]) # see which coefficients were significant in the top model
...
bold.model.global = clm(Bold ~ Road_Distance_Decay + Natural + Modified_Open + Mowed + Commercial + Building_Density + Season + 
                        Road_Distance_Decay*Season + Modified_Open*Season + Mowed*Season + 
                        Commercial*Season  + 
                          Year + 
                          Road_Distance_Decay*Year + Mowed*Year + Building_Density*Year + Modified_Open*Season*Year, # interactions with year
                      data = bold.data.modelling, na.action = "na.fail")

AIC(bold.model.global) - AIC(bold.model.null) 

bold.model.global.dredge = dredge(bold.model.global)# dredge the global model

bold.models.list = get.models(bold.model.global.dredge, subset = delta < 2) # get the list of top global models
...
rm(bold.data.modelling, bold.models.list, bold.model.global, bold.model.base.dredge.coefs, bold.model.base.dredge.variables, bold.model.base, bold.models.list)
```

Writing a CSV with coefficients, confidence intervals (95%), AIC, deltaAIC, weights, and k for each of the models that made it within 2 AIC points of the top model
```{r}
mod.outputs.bold %>%
  mutate(coef_confint = paste(round(odds, 2), # combining coefficients and confidence intervals into one column 
                              gsub(" ","", paste("(",round(conf_2.5, 2),",",round(conf_97.5,2), ")")))) %>%
  pivot_wider(id_col=AIC, names_from=variable, values_from = coef_confint) %>%
  mutate(delta = round(bold.model.global.dredge[1:20,]$delta, 2),# add # parameters to model 
         k =bold.model.global.dredge[1:20,]$df, # add delta AIC to model
         weight = round(bold.model.global.dredge[1:20,]$weight, 2)) %>% # add AIC weights to model 
  write.csv("Output_Tables/coyote_boldness_regression_outputs.csv") # write csv with model outputs for all top models

rm(bold.model.global.dredge)
```
...
Third, we select the best-fit scale using univariate ordinal regressions. Based on the AIC values of univariate models, the best scales are Res800m, Mod1600m, Nat1600m, Com100m, Mow800m. We removed RoadDistDecay and Building density as their AIC values were worse than the null model
```{r}
...
rm(perc.scale.models, perc.scale.models.outputs)
```

Fourth, we evaluate correlations between our variables using R-squared tests. No big corrs. We also briefly write a csv file with our correlation ouptut.
```{r}
concern.correlations = rcorr(as.matrix(perc.data.trans[,c("Mod1600m", "Mow800m", "Nat1600m", "Com100m", "Res800m")]), type = c("spearman"))
...
perc.model.base = clm(HumanPerception ~ Natural + Modified_Open + Mowed + Commercial + Residential + Season + 
                        Natural*Season + Modified_Open*Season + Mowed*Season + Commercial*Season + Residential*Season, 
                      data = perc.data.modelling, na.action = "na.fail")

AIC(perc.model.base) - AIC(perc.model.null)

perc.model.base.dredge = dredge(perc.model.base) # dredge to get the top models 
perc.model.base.dredge.coefs <- model.avg(perc.model.base.dredge, subset=delta < 2) # get the top models
summary(perc.model.base.dredge.coefs) # see the variables that made it into the top models


perc.model.base.dredge.variables <- get.models(perc.model.base.dredge, subset=delta ==0) # get the top model
summary(perc.model.base.dredge.variables[["51"]]) # see which coefficients made it into the top models WITH sig effects
...
# interactions between year and the following: Modified_Open, Residential, Road_Distance_Decay

# interactions only if significant effects existed in base model
perc.model.global = clm(HumanPerception ~  Natural + Modified_Open + Mowed + Commercial + Residential + Season + 
                          Year +  Modified_Open*Year + Residential*Year,
                      data = perc.data.modelling, na.action = "na.fail")
AIC(perc.model.global) - AIC(perc.model.null)

summary(perc.model.global)

perc.model.global.dredge = dredge(perc.model.global) # dredging to get the models

perc.models.list = get.models(perc.model.global.dredge, subset = delta < 2) # 42 top models 
...
rm(perc.model.base, perc.model.base.dredge, perc.model.base.dredge.coefs, perc.model.base.dredge.variables, perc.model.global,  perc.models.list)
```

Writing a CSV with coefficients, confidence intervals (95%), AIC, deltaAIC, weights, and k for each of the models that made it within 2 AIC points of the top model
```{r}
mod.outputs.perc %>%
  mutate(coef_confint = paste(round(odds,5), # combining coefficients and confidence intervals into one column 
                              gsub(" ","", paste("(",round(conf_2.5,5),", ",round(conf_97.5,5), ")")))) %>%
  pivot_wider(id_col=AIC, names_from=variable, values_from = coef_confint) %>%
  mutate(delta = perc.model.global.dredge[1:20,]$delta, # add delta AIC to model
         k =perc.model.global.dredge[1:20,]$df, #  add # parameters to model 
         weight = perc.model.global.dredge[1:20,]$weight, ) %>% # add AIC weights to model 
  write.csv("Output_Tables/human_concern_regression_outputs.csv") # write csv with model outputs for all top models

rm(perc.model.global.dredge)
```

### Final plot for ordinal regression models
...
ordinal.outputs$variable = factor(ordinal.outputs$variable, 
                               levels = c( "Modified_Open:SeasonDispersal", "Modified_Open:SeasonPupRearing",
                                          ...
                                          "Year"),
                               labels = c( "Modified Open:Dispersal Season","Modified Open:Pup Rearing Season",
                                          "Modified Open","Mowed","Residential:Year", "Residential","Building Density", "Road Distance Decay", 
...
save_plot("Figures/ordinal.regression.outputs.jpeg", ordinal.output.plot, base_width = 10, base_height= 6)

rm(mod.outputs.bold, mod.outputs.perc, bold.output.top.mod, perc.output.top.mod, ordinal.outputs, ordinal.output.plot)
```
...
Boldness & Human concern across land cover types - Figure showing trends as percentages
```{r}
...
Boldness & Human concern across land cover types - Looking at chi square relationship and visualizing pearson's residuals. One figure (for coyote boldness only) 
```{r}
...
### Temporal Patterns in Boldness & Human concern based on percentage data

Over months & coyote seasons (figure as percentages)
```{r}
...
Boldness & concern over diel scales (figure as percentages) 
```{r}
...
### Patterns in Boldness & Human Concern indicated by percentages - for Table X
...
# Dog leash status
bold.leash =  Coyotes %>% filter(Bold != "Unknown", Bold  != "Sighting", DogPresent !="HomeYard") %>%
...
perc.leash = Coyotes %>% filter(HumanPerception !="Concern", HumanPerception!="Unknown", DogPresent !="HomeYard")%>%
...
bold.perc = Coyotes %>% 
  filter(Bold!="Sighting" & Bold!="Unknown" & HumanPerception !="Concern" & HumanPerception!="Unknown")
 # total of 653 reports where both perception & boldness could be determined
...
ggplot(bold.per.chsq.resids.long, aes(x = HumanConcern, y = Bold, fill = Resids)) +
  ...
  scale_fill_distiller("Pearson's\nResiduals",palette = "RdYlBu", na.value = "grey80", 
                       limits =c(-5,5), breaks = c(-5,0,5), labels = c(-5, 0, 5), direction=1)+ 
  ylab("Coyote Boldness") + xlab("Human Concern") + 
  theme_bw()+   
  scale_x_discrete(expand =c(0,0))+ scale_y_discrete(expand = c(0,0))+
  theme(legend.position = "right", legend.title = element_text(size = 20), legend.text = element_text(size = 18),
        axis.text.x = element_text(size = 18), axis.title= element_text(size = 20),
        axis.text.y = element_text(size = 18))

    save_plot("Figures/correlation.between.bold.concern.jpeg", bold.per.chsq.plot, base_width = 8, base_height= 8)

    
    rm(bold.per.chsq.plot, bold.perc, bold.per.table, bold.per.chsq,bold.per.chsq.resids, bold.per.chsq.resids.long)
```
...
### Supplementary Table: X-square test outputs for correlations
...
vul.leash.table = table(Coyotes$VulnerableIndividual, Coyotes$DogPresent) # remove cat as <5 expected
...
act.leash.table =  table(Coyotes$HumanActivity, Coyotes$DogPresent) # remove cycling and outdoor act as they have < 5 expected reports
act.leash.chsq = chisq.test(act.leash.table[c(2,3,5,6),])

act.health.table =  table(Coyotes$HumanActivity, Coyotes$Health) # remove cycling as some categories < 5 expected reports
act.health.chsq = chisq.test(act.health.table)
...
leash.num.table =  table(Coyotes$DogPresent, Coyotes$CoyoteNumber) 
leash.num.chsq = chisq.test(leash.num.table)

leash.health.table =  table(Coyotes$DogPresent, Coyotes$Health) 
...
szn.leash.table =  table(Coyotes$Season, Coyotes$DogPresent)
...
diel.leash.table =  table(Coyotes$NightDay, Coyotes$DogPresent)
...
### Supplementary Table X: X square test outputs for relationships between categorical variables and boldness/perceptions
...
bold.act.chsq = chisq.test(bold.act.table[3:6,]) 

bold.leash.table = table(Coyotes$Bold, Coyotes$DogPresent)
...
perc.leash.table = table(Coyotes$HumanPerception, Coyotes$DogPresent)
perc.leash.chsq = chisq.test(perc.leash.table[2:4,2:4]) 
...
# cleaning up workspace
rm(bold.hab.table, bold.diel.table, bold.vul.table, bold.act.table, bold.leash.table, bold.num.table, bold.health.table,
                         perc.hab.table, perc.diel.table, perc.vul.table, perc.act.table, perc.leash.table, perc.num.table, perc.health.table, perc.bold.table)

# making a list to input to the loop to extract results
conflict_chsq_list = list(bold.hab.chsq, bold.diel.chsq, bold.vul.chsq, bold.act.chsq, bold.leash.chsq, bold.num.chsq, bold.health.chsq,
                         perc.hab.chsq, perc.diel.chsq, perc.vul.chsq, perc.act.chsq, perc.leash.chsq, perc.num.chsq, perc.health.chsq, perc.bold.chsq)
...
# cleaning up the workspace
rm(bold.hab.chsq, bold.diel.chsq, bold.vul.chsq, bold.act.chsq, bold.leash.chsq, bold.num.chsq, bold.health.chsq,
                         perc.hab.chsq, perc.diel.chsq, perc.vul.chsq, perc.act.chsq, perc.leash.chsq, perc.num.chsq, perc.health.chsq, perc.bold.chsq)
